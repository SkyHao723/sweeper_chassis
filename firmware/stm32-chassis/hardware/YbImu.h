...
/* ★ 加速度和角速度分两次读, 各 6 字节 —— 和厂商的参考实现
 *   (IMU_I2C_ReadAccelerometer / IMU_I2C_ReadGyroscope) 一致。
 *
 *   历史: 曾经把"角速度恒为 0"归因于"一次读 12 字节, 地址指针不跨功能块
 *   自增"。**那个归因很可能是错的** —— 实测发现这颗模块**在检测到静止时
 *   本来就把角速度输出归零**(静止 478 帧全零, 转动 321 帧都有数据),
 *   所以当时看到的"陀螺仪恒 0"多半只是"读的时候没动它"。
 *   两次读继续保留(和厂商一致, 没有坏处), 但别再把 12 字节读当成故障原因。 */
...
/* 6 号以前是 YBIMU_ST_GYRO_ZERO("加速度正常但角速度恒为 0")。
   **那个判定是错的, 已删除**(见 YbImu.c 里 YbImu_ReadMotion 的说明):
   模块静止时角速度本来就输出 0, 那不是故障。单看一瞬间也分不出"坏了"和
   "正好静止" —— 判断陀螺仪死活只能靠"动一下看数值变不变"。
   不要把这个码加回来。 */

/* 读一次运动数据。返回 YBIMU_ST_OK 或 YBIMU_ST_* 失败码。
   注意: 静止时角速度会是 0, 这是模块的正常行为, 不是失败。 */
