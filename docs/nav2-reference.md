...
| **本次导航速度上限** | `vx ≤ 0.30~0.35 m/s`，`|wz| ≥ 0.35 rad/s`（理由见 §3） |
| **发布 `/cmd_vel` 的频率** | **必须 ≥ 2 Hz 持续发**（8 百毫秒不发，底盘自己刹车） |

**四条最容易踩的**：

1. **`odom` 帧没有 TF**，只有 `odom_combined` 有。Nav2 的 `odom_topic` 用
   `/odom_combined`，`odom_frame` 写 `odom_combined`。
2. **底盘不保持最后一条命令。** 800 ms 没收到速度帧 → 目标归零 + 刹车 + 锁存
   （诊断帧 `flags` bit1）。所以**你的控制器必须按固定频率持续发 `/cmd_vel`**；
   发一条就退出，车只走 0.8 秒。Nav2 默认 20 Hz，没问题；自己写的发布节点要注意。
3. **阶跃起步会过冲**：你发 0.30，车会先冲一下再落回（历史实测峰值约 138%）。
   固件在 2026-09 改过起步助推，过冲已经压下去，但**还没重测**（见 §2.4）。
   Nav2 的限速、以及任何"按时间估算应走多远"的检查都要容忍这个。
4. **停车后会滑约 12 cm**（低速时刹车力弱）。`goal` 的容差要留够。
...
- ⚠⚠ 更要紧的：这几档的**起步助推**在 2026-09 换掉了（见 §2.4）。旧助推是
  固定 +12 RPM 加性量，等效于每次原地转起步都**额外加 0.28 rad/s**，
  也就是说上表 0.50 档起步那 400 ms 真实 wz 约 0.78（155%）。新固件按缺口给量，
  `|wz| ≥ 0.35` **完全不加助推**。所以**上表需要重测**；预计稳态不变、起步段变平。

### 2.4 起步助推（2026-09 改动，★ 未上车验证）

实车报过两个症状，**是同一个根因的两端**：

| 症状 | 根因 |
|---|---|
| 发小一点的转向命令，车愣在原地 | 旧助推是 **400 ms 一次性定时**，超时就撤，且目标没回零就**不会重新武装** |
| 转向太敏感 | 旧助推是**固定 +12 RPM 加性**，原地转时等效 **+0.28 rad/s**；每次起步都给满 400 ms |

新逻辑（`KICK_*` 宏，都在 `user/main.c`）：

```
助推 = clamp(15 - 较小的那个|名义轮速|, 0, 20)   RPM
只在 |实际轮速| < 2 RPM 时给;  |实际| > 6 RPM 立刻撤
补到 15 还不动 -> 每 200ms 再加 4 RPM, 到 20 封顶
加到 20 还不动满 3 秒 -> 歇 2 秒再试(不是永久放弃), 诊断帧 flags bit7 锁存置位
```

对导航的直接含义：

- **`|wz| ≥ 0.35 rad/s`（单轮名义 ≥15 RPM）：完全不加助推，干净。**
- `|wz| < 0.35`：从静止起步时助推会把 wz **临时**抬高（0.25 档约 +0.10 rad/s）。
  这不是 bug，是**破静摩擦的物理代价** —— 这个驱动器从静止起步就是要约
  15 RPM 才动得起来。
- 因此 **Nav2 的 `min_speed_theta` 建议 0.35**，并且尽量避免让 Nav2 规划出
  "原地几乎不转"的微调动作。
- **边走边转（两轮名义值同号）不受偏航影响**：助推给两轮加的是同一个方向，
  差分不变 → `wz` 诚实，只是 `vx` 会短暂冲高一点。所以"转向变敏感"只可能来自
  **原地转**，直行/弧线里不用担心。
- 调试时看诊断帧 `flags`：**bit6 = 此刻正在给助推**，bit7 = 加到上限还不转。
  两者都能用 `tools/decode_diag.py` 直接读出来。
...
| **`\|wz\| < 0.35 rad/s`** | 单轮名义 <15 RPM，**从静止起步要靠助推才动**（见 §2.4） | 设成 Nav2 的最小角速度 |
...
max_vel_theta: 1.0
min_speed_theta: 0.35             # ★ 单轮 15 RPM, 低于它从静止起步要靠助推(§2.4)
# 停车滑行约 12 cm
