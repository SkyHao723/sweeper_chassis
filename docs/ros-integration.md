...
所以它对"没做到位"一无所知。现在加了一层 `Trim_Apply()`：**只含积分**的慢外环，
按驱动器回传的实际转速去修目标值（不做比例项 —— 那等于把驱动器那个环照抄一遍，
两个快环叠加容易互相激振）。

修正量限幅 ±30 RPM 后再整体限到 ±`MAX_RPM`；**目标为 0 时恒定输出 0**，所以它
不可能在停车指令下把车带着走。实现细节和调参见 `README.md` 的"轮速外环修正"。
...
| **转速控制精度** | 低速 64%、常用档 103%、高速 134%；原地转 4 秒也只到 71~82% | **STM32 已加轮速外环修正**（4.6 节 / README）：按驱动器回传的实际转速修目标值，限幅且停车时恒定输出 0。**待烧录验证**。偏差属**底盘执行**问题、不是融合问题 |
