...
- **软肋在驱动器的低速环**：在 STM32 上加一层**慢速外环修正**（按驱动器回传的
  实际转速去修目标值，慢积分 + 硬限幅），能同时改善稳态欠速和左右不对称。
  风险是外层和内层振荡互斗，必须限幅 + 丢帧保护，改之前要留退路。
- **绕开死区**：由 4.5 的曲线，轮速低于约 20 RPM（≈0.22 m/s 线速、
  ≈0.47 rad/s 原地转）时驱动器就开始不稳定，导航里尽量别用比这更慢的动作。
...
| **转速控制精度** | 低速 64%、常用档 103%、高速 134%；原地转 4 秒也只到 71~81% | **已按包线规避**：导航速度限在 0.25~0.35 m/s（4.5 节）。原地转起步要 1.5~2 秒（4.6 节），偏差属**底盘执行**问题、不是融合问题。根治要么找厂家调驱动器速度环，要么在 STM32 上加慢速外环修正 |
