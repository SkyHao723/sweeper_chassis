...
所以它对"没做到位"一无所知。现在加了一层：`Boost_Decide()`（起步助推，两轮共享
一次判决）+ `Trim_Apply()`（慢积分外环），按驱动器回传的实际转速去修目标值。

- **积分**只修稳态偏差（`TRIM_KI=0.3`），而且**只在误差小于 `TRIM_LIN_BAND_RPM=5`
  RPM 时才积分** —— 起步/被刮擦卡住时误差有 20~28 RPM，那时积分会 windup 到限幅，
  然后和驱动器的 PI 组成 0.25 Hz 极限环（实测 `目标-理论` 在 ±14 RPM 来回摆）。
- **助推**专门对付起步的静摩擦（那需要"立刻给力"，而积分天生要等误差攒起来）。
  判据是**绝对转速**：`|实际| < 2 RPM` 就给，`|实际| > 6 RPM` 立刻撤；量是
  `clamp(15 - 最小|名义轮速|, 0, 20)` RPM，补到 15 还不动就每 200 ms 再加 4 RPM。
  于是 **`|wz| ≥ 0.35`（单轮 ≥15 RPM）和直行 0.30 m/s 都完全不加助推**。
- **限幅按名义值比例**（`+20% / -20%`），所以指令永远落在 `[0.8, 1.2]` 倍名义值
  之内、**方向永远不变**。第一版用绝对 ±30 RPM，在名义值只有 13 RPM 的原地转上
  把指令顶成了 `13-30 = -17`，**轮子反转**。

**症状与根因**（实车报的两个毛病是同一个根因的两端）：

| 症状 | 根因 |
|---|---|
| 小转向命令愣在原地 | 助推曾是 **400 ms 一次性定时**，超时即撤，目标没回零就**不会重新武装** |
| 转向太敏感 | 助推曾是**固定 +12 RPM 加性**，原地转等效 **+0.28 rad/s**，每次起步给满 400 ms |

**目标为 0 时恒定输出 0**，所以它不可能在停车指令下把车带着走。实现细节、逐版
事后分析、调参方法见 `README.md` 的"轮速外环修正"。
...
- **第四版外环（2026-09：绝对转速助推判据）还没上车验证**，`KICK_BASE_RPM` /
  `KICK_STEP_*` / `TRIM_KI` 都还是首猜值。`|wz| ≥ 0.35` 完全不助推是**设计意图**，
  但"直行 0.30 m/s 在助推为 0 时能不能可靠起步"必须实测确认（旧版是靠 +12 RPM
  起步的，现在不再给）。
